...
# # response.data is a list of dictionaries, so we iterate through it directly:
# # for row in response.data:
# #     # Safely grab the state and district_id from the dictionary
# #     fips = int(row['state'])
# #     dist_id = int(row['district_id']) 

# for row in response.data:
#     if not row.get('state') or not row.get('district_id'):
#         continue
...
#                 # Cast API ID to integer to safely match our database ID (drops leading zeros)
#                 api_dist_id = int(dist["districtID"])
                
#                 if api_dist_id in needed_ids:
#                     rank_history = dist.get("rankHistory", [])
                    
#                     if rank_history:
#                         # Safely grab the most recent ranking year
#                         latest_rank = max(rank_history, key=lambda x: x.get("year", 0))
                        
#                         # Prepare the payload matching your database columns exactly
#                         insert_data = {
#                             "district_id": str(api_dist_id),                 # 1. Your internal ID
#                             "schooldigger_district_id": dist["districtID"],  # 2. The exact API string
#                             "state": state,
...
#             # Stop turning pages if we found all targets for this state
...
#         page += 1
#         time.sleep(0.5) # Be polite to the free API rate limits

#     # Safety coverage logging
#     if districts_found < len(needed_ids):
